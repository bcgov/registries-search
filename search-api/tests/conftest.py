...
@pytest.fixture(scope='session')
def db_flag(app):  # pylint: disable=redefined-outer-name, invalid-name
    """Return a session-wide initialised database.
...
@pytest.fixture(scope='function')
def session_flag(db_flag):  # pylint: disable=redefined-outer-name, invalid-name
    """Return a function-scoped session."""
    conn = db_flag.engine.connect()
    txn = conn.begin()

    options = dict(bind=conn, binds={})
    sess = db_flag.create_scoped_session(options=options)
