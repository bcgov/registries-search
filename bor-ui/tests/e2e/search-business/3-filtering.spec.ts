...
test.describe('Search Business - filtering', () => {
  ...
  test('Filtering triggers expected api calls', async ({ page }) => {
    const verifyFilterRequest = async (
      ...
      const searchRequest = await page.waitForRequest('**/api/v2/search/businesses', { timeout: 5000 })
    ...
    await verifyFilterRequest(nameFilterValue, identifierFilterValue)
    // test filter bn
    const bnFilterValue = '0'
    await triggerTextFilter(headerFilters.at(2) as Locator, bnFilterValue)
    await verifyFilterRequest(nameFilterValue, identifierFilterValue, bnFilterValue)
    ...
    await verifyFilterRequest(nameFilterValue, identifierFilterValue, bnFilterValue, ['BEN'])
    ...
    await verifyFilterRequest(nameFilterValue, identifierFilterValue, bnFilterValue, legalTypeFilterValue)
    // test filter status
    await triggerSelectFilter(headerFilters.at(4) as Locator, 'Active')
    await verifyFilterRequest(nameFilterValue, identifierFilterValue, bnFilterValue, legalTypeFilterValue, ['ACTIVE'])
    // clear all filters
