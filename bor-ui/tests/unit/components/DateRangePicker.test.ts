...
import BaseDatePicker from '../../../app/components/base/datePicker/Index.vue'
...
describe('BC Reg Date Range Picker tests', () => {
  ...
  it('renders and displays the date range picker', () => {
    ...
    // buttons
    expect(wrapper.findAll('[data-testid="date-selection-btn"]').length).toBe(2)
    expect(wrapper.findAll('[data-testid="date-selection-btn"]')[0].text()).toBe('OK')
    expect(wrapper.findAll('[data-testid="date-selection-btn"]')[1].text()).toBe('Cancel')
  ...
  it('Validates and submits the date range selections', async () => {
    ...
    let submitButtons = wrapper.findAll('[data-testid="date-selection-btn"]')
    ...
    submitButtons = wrapper.findAll('[data-testid="date-selection-btn"]')
    ...
    expect(wrapper.vm.datePickerErr).toBe(false)
    expect(wrapper.emitted('submit')?.length).toBe(1)
    expect(wrapper.emitted('submit')[0]).toEqual([{ startDate, endDate }])
  })
})
