    ...
    def connect(self) -> Callable:
        ...
        if self.client is None:
            try:
                ...
                logging.error(f'GoogleCloudStorage.connect() failed: {err}')
                raise StorageServiceError(  # pylint: disable=raise-missing-from
                    f'GoogleCloudStorage.connect() failed: {err}', e=err)
    ...
    def save_document(self,
        ...
        try:
            ...
            if doc_type == StorageDocumentTypes.BINARY:
                ...
                raise StorageServiceError(f'Unsupported document type: {doc_type}')

            gcs_file.write(raw_data)
            gcs_file.close()
        except Exception as err:  # noqa: B902
            logging.error(f'GoogleCloudStorage.save_document() failed: {err}')
            raise StorageServiceError(  # pylint: disable=raise-missing-from
                f'GoogleCloudStorage.save_document() failed: {err}', e=err)
