...
integration_nats = pytest.mark.skipif(
    get_skip_value("RUN_NATS_TESTS"), reason="NATS tests are skipped when RUN_NATS_TESTS is False."
)

integration_payment = pytest.mark.skipif(
    get_skip_value("RUN_PAYMENT_TESTS"),
    reason="Test requiring payment service are skipped when RUN_PAYMENT_TESTS is False.",
)

integration_ldarkly = pytest.mark.skipif(
    get_skip_value("RUN_LD_TESTS"), reason="Launch Darkly integration tests are skipped when RUN_LD_TESTS is False."
)

integration_solr = pytest.mark.skipif(
    get_skip_value("RUN_SOLR_TESTS"), reason="SOLR integration tests are skipped when RUN_SOLR_TESTS is False."
)

not_github_ci = pytest.mark.skipif(get_skip_value("NOT_GITHUB_CI"), reason="Does not pass on github ci.")
