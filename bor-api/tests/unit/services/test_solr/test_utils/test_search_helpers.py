...
BASIC_PAYLOAD = expected_payload = {
    'query': '(legalName_q:name^2 OR legalName_q:name~1 OR legalName_stem_agro_q:name^2 OR legalName_stem_agro_q:name~1 OR legalName_single_term_q:name^2 OR legalName_single_term_q:name~1 OR identifier_q:name OR bn_q:name^2 OR bn_q:name~1 OR ({!parent which = \'-_nest_path_:* entityType:*\'}address_q:name~1)) OR (legalName_q:"name"~5^5) OR (legalName_stem_agro_q:"name"~10^3) OR (legalName_synonym_q:"name"~10^3) OR (legalName_stem_agro_q:"name"^2)',
    'filter': [],
    ...
    'facet': {'entityType': {'type': 'terms', 'field': 'entityType'},
                ...
                'roleType': {'type': 'terms',
                            ...
                            'domain': {'blockChildren': '{!v=$parents}'},
                            'facet': {'by_parent': 'uniqueBlock({!v=$parents})'}}},
    'fields': ['bn', 'entityAddresses', 'entityType', 'identifier', 'legalName', 'legalType', 'roles', 'state', 'score', '[child]', 'addressCity', 'addressCountry', 'addressRegion', 'addressType', 'postalCode', 'streetAddress', 'active', 'relatedBN', 'relatedEntityType', 'relatedIdentifier', 'relatedName', 'relatedState', 'roleDates', 'roleType', 'relatedLegalType', 'start', 'end']}

COMPLEX_PAYLOAD = {
    'query': '(legalName_q:name1^2 OR legalName_q:name1~1 OR legalName_stem_agro_q:name1^2 OR legalName_stem_agro_q:name1~1 OR legalName_single_term_q:name1^2 OR legalName_single_term_q:name1~1 OR (identifier_q:"1" AND identifier_q:"NAME") OR bn_q:name1^2 OR bn_q:name1~1 OR ({!parent which = \'-_nest_path_:* entityType:*\'}address_q:name1~1)) AND (legalName_q:name2^2 OR legalName_q:name2~1 OR legalName_stem_agro_q:name2^2 OR legalName_stem_agro_q:name2~1 OR legalName_single_term_q:name2^2 OR legalName_single_term_q:name2~1 OR (identifier_q:"2" AND identifier_q:"NAME") OR bn_q:name2^2 OR bn_q:name2~1 OR ({!parent which = \'-_nest_path_:* entityType:*\'}address_q:name2~1)) AND (legalName_q:name3^2 OR legalName_q:name3~1 OR legalName_stem_agro_q:name3^2 OR legalName_stem_agro_q:name3~1 OR legalName_single_term_q:name3^2 OR legalName_single_term_q:name3~1 OR (identifier_q:"3" AND identifier_q:"NAME") OR bn_q:name3^2 OR bn_q:name3~1 OR ({!parent which = \'-_nest_path_:* entityType:*\'}address_q:name3~1)) OR (legalName_q:"name1 name2 name3"~5^5) OR (legalName_stem_agro_q:"name1 name2 name3"~10^3) OR (legalName_synonym_q:"name1 name2 name3"~10^3) OR (legalName_stem_agro_q:"name1"^2)',
    'filter': ['(identifier_q:"12345" AND identifier_q:"BC")', 'legalName_single_term_q:Test', 'state:("ACTIVE" OR "HISTORICAL")', 'entityType:("PERSON" OR "BUSINESS")', '({!parent which = \'-_nest_path_:* entityType:*\'}address_q:vancouver AND {!parent which = \'-_nest_path_:* entityType:*\'}address_q:bc AND {!parent which = \'-_nest_path_:* entityType:*\'}relatedBN_q:123 AND ({!parent which = \'-_nest_path_:* entityType:*\'}related_q:"0012345" AND {!parent which = \'-_nest_path_:* entityType:*\'}relatedIdentifier_q:"S") AND {!parent which = \'-_nest_path_:* entityType:*\'}related_q:name)', '{!parent which = \'-_nest_path_:* entityType:*\'}addressCity:"North Vancouver" OR addressCity: "Victoria"', '{!parent which = \'-_nest_path_:* entityType:*\'}relatedState:"ACTIVE"', "{!parent which = '-_nest_path_:* entityType:*'}start:[* TO *] AND (end:[2022-03-21 TO *] OR active: true)"],
