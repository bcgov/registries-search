...
class Solr:
    ...
    def call_solr(  # noqa: PLR0913
        ...
        retries = Retry(
            total=1, backoff_factor=1, status_forcelist=[500, 502, 503, 504], allowed_methods=["GET", "POST"]
        )
        session = Session()
        ...
        try:
            if method == "GET":
                ...
                response = session.post(
                    url=url, data=xml_data, headers={"Content-Type": "application/xml"}, timeout=timeout
                )
            else:
            ...
            if response.status_code != HTTPStatus.OK:
                raise Exception(response.json().get("error", {}).get("msg", "Error handling Solr request."))
            ...
            raise SolrException(
                error="Connection error while handling Solr request.", status_code=HTTPStatus.GATEWAY_TIMEOUT
            ) from err
    ...
    def replication(self, command: str, leader=True):
        ...
        resp = self.call_solr(method="GET", query=self.replication_url, params={"command": command}, leader=leader)
