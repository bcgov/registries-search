...
def _is_good_standing(item_dict: dict, source: str) -> bool:  # noqa: PLR0911
    ...
    if item_dict["state"] != "ACTIVE":
        # Good standing is irrelevant to non-active businesses and FIRMS
        return None
    if source == "LEAR":
        ...
        return last_file_date + datedelta(years=1, months=2, days=1) > datetime.now(tz=UTC)
...
def prep_data(data: list, data_descs: list[str], source: str) -> list[dict]:  # noqa: PLR0912
    ...
    for item in data:
        item_dict = dict(zip(data_descs, item, strict=False))
    ...
    solr_docs = []
    for base_doc in prepped_data.values():
